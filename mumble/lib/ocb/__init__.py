...
mode OCB (Offset Codebook Mode) using AES block cipher. OCB offers
...
class OcbTestCase(unittest.TestCase):
    ...
    def test_pmac1_2(self):
        ...
        ocb.setKey(bytearray(range(16)))
        header = bytearray(range(90))
        out = ocb._pmac(header)
    ...
    def test_ocb(self):
        for key_len in (128, 192, 256):
            ...
            plaintext = bytearray('The Magic Words are Squeamish Ossifrage'.encode("ascii"))
            header = bytearray('Recipient: john.doe@example.com'.encode("ascii"))
